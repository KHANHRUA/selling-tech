<template>
  <div class="Container">
    <div class="nav">
      ...
      <h2 v-for="(part, index) in nav" :key="index" class="path">/ {{ part.name }}</h2>
    </div>
    <div class="temp1">
      <div class="left">
        <div class="CoreIndex">
      ...
      <div class="right">
        <div class="product">
          <div id="imgContain">
            <img id="featured" :src="`${Object.data.url}`" alt="error" class="img" />
            <div id="lens" @wheel="handleMouseWheel"></div>
          </div>
          <div class="information">
            <div id="view"></div>
            <div class="name">{{ Object.data.name }}</div>
            <div class="Rate">
              <div
                v-for="n of Array.from(
                  { length: Object.data.rate / 2 },
                  (_, index) => index + 1
                )"
                :key="n"
              >
              ...
              <div v-if="Object.data.rate % 2 === 1">
                <StarHalf></StarHalf>
              </div>
              <div v-if="Object.data.rate % 2 === 0 && Object.data.rate / 2 != 5">
                <StarEmpty></StarEmpty>
              </div>
              <div
                v-for="n of Array.from(
                  { length: 4.5 - Object.data.rate / 2 },
                  (_, index) => index + 1
                )"
                :key="n"
              >
            ...
            <div class="Price">Giá bán: {{ Object.data.price }} VNĐ</div>
            <div class="Id">Mã sản phẩm: {{ Object.data.name }}</div>
            <div class="quantity">
            ...
            <div class="Order" @click="AddToCart(Object.data)">ĐẶT HÀNG</div>
          ...
          <div class="Swiper">
            <div class="swiper">
              ...
              <div class="swiper-wrapper containSwiper">
                ...
                <div
                  class="swiper-slide"
                  v-for="(part, index) in Object.picture"
                  :key="index"
                  @click="changeImg(part.url)"
                >
        ...
        <div class="Nav">
          <div :class="Nav == 1 ? 'Text1' : 'Text'" @click="changeComponent(1)">
            GIỚI THIỆU
          </div>
          <div :class="Nav == 2 ? 'Text1' : 'Text'" @click="changeComponent(2)">
            THÔNG SỐ KỸ THUẬT
          </div>
          <div :class="Nav == 3 ? 'Text1' : 'Text'" @click="changeComponent(3)">
            ĐÁNH GIÁ SẢN PHẨM
          </div>
          <div :class="Nav == 4 ? 'Text1' : 'Text'" @click="changeComponent(4)">
            TIN TỨC LIÊN QUAN
          </div>
        </div>
        <div v-if="Nav == 1">Description</div>
        <div v-else-if="Nav == 2">... Đang được cập nhật</div>
        <div v-else-if="Nav == 3">
          <div class="judge">
            Đánh giá sản phẩm
            <div class="starList">
              <div v-for="n of Array.from({ length: Rate }, (_, index) => index + 1)" :key="n">
                <Star class="starYellow" @mousemove="setStar(n)"></Star>
              </div>
              <div
                v-for="n of Array.from({ length: 5 - Rate }, (_, index) => index + 1)"
                :key="n"
              >
                <Star class="star" @mousemove="setStar(Rate + n)"></Star>
              </div>
            </div>
          </div>
          <div class="comment">
        ...
        <div v-else>Không có tin tức nào liên quan đến sản phẩm !</div>
        <span class="facebook"><iframe name="fce1951f8e958c" height="100px" data-testid="fb:comments Facebook Social Plugin" title="fb:comments Facebook Social Plugin" frameborder="0" allowtransparency="true" allowfullscreen="true" scrolling="no" allow="encrypted-media" src="https://web.facebook.com/v2.12/plugins/comments.php?app_id=&amp;channel=https%3A%2F%2Fstaticxx.facebook.com%2Fx%2Fconnect%2Fxd_arbiter%2F%3Fversion%3D46%23cb%3Df335a8e9d244308%26domain%3Dthaituan-hobby.com%26is_canvas%3Dfalse%26origin%3Dhttp%253A%252F%252Fthaituan-hobby.com%252Ff28bb4814e767c8%26relation%3Dparent.parent&amp;color_scheme=light&amp;container_width=392&amp;height=100&amp;href=http%3A%2F%2Fthaituan-hobby.com%2Fthiec-han-sanki-6040-0.8mm-250g-p37882120.html&amp;locale=vi_VN&amp;mobile=true&amp;numposts=5&amp;sdk=joey&amp;version=v2.12&amp;width=" style="border: none; visibility: visible; height: 213px;" class=""></iframe></span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    moveLens() {
      ...
      this.temp.style.backgroundPosition = `-${positionLeft * this.ratio}px -${
        positionTop * this.ratio
      }px`
    },
    getCursor() {
