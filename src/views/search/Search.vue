      ...
      <div class="right">
        <div class="Sort">
          ...
          <Stack
            class="Stack"
            @click="set(0)"
            :style="{ color: display ? 'black' : 'red' }"
          ></Stack>
          <Rubik
            class="Rubik"
            @click="set(1)"
            :style="{ color: display ? 'red' : 'black' }"
          ></Rubik>
          Tìm kiếm theo
        ...
        <div v-if="display" class="CoreData">
          <div v-for="(part1, index1) in Object[page]" :key="index1" class="Product">
        ...
        <div v-else class="CoreData1">
          <div v-for="(part1, index1) in Object[page]" :key="index1" class="Product1">
            <img :src="`${part1.url}`" alt="error" class="Img" />
        ...
        <div v-if="modal" class="ContainerModal">
          <div class="outside" @click="Off()"></div>
          <div class="modal">
            <div class="Picture">
              <img :src="`${temp.url}`" alt="error" class="Img" />
            </div>
            <div class="in4">
              <div class="name">{{ temp.name }}</div>
              <div class="rate">
                <div
                  v-for="n of Array.from({ length: temp.rate / 2 }, (_, index) => index + 1)"
                  :key="n"
                >
                  <Star class="icon"></Star>
                </div>
                <div v-if="temp.rate % 2 === 1">
                  <StarHalf class="icon"></StarHalf>
                </div>
                <div v-if="temp.rate % 2 === 0 && temp.rate / 2 != 5">
                  <StarEmpty class="icon"></StarEmpty>
                </div>
                <div
                  v-for="n of Array.from(
                    { length: 4.5 - temp.rate / 2 },
                    (_, index) => index + 1
                  )"
                  :key="n"
                >
              ...
              <div class="price">{{ temp.price }} VNĐ</div>
              <div class="id">Mã sản phẩm: {{ temp.name }}</div>
              <div class="quantity">
                ...
                <div class="Order" @click="AddToCart(temp)">ĐẶT HÀNG</div>
              </div>
            </div>
        ...
        <div class="changePage">
          <div
            v-for="(part, index) in pageMax"
            :key="index"
            class="Number"
            @click="changePage(index)"
            :style="{ 'background-color': page == index ? '#ccc' : '' }"
          >
            {{ index + 1 }}
          </div>
          <div class="Number" @click="changePage(999)" v-if="pageMax != 0">
            <Triangle></Triangle>
          </div>
        </div>
      </div>
    ...
    handleMouseMove1(event) {
      ...
      if (volumePercentage >= this.percentMin && volumePercentage <= 1) {
        ...
        line.style.marginRight = `${
          volumeControl.offsetWidth - (event.clientX - volumeControl.getBoundingClientRect().left)
        }px`
      }
    },
    endDrag() {
